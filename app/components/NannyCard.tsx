...
export const NannyCard = ({ nanny }: NannyCardProps) => {
  ...
  const features = [
    ...
    {
      label: "Characters",
      value: nanny.characters
        .join(", ")
        .replace(/(^\w|\s\w)/g, (match) => match.toUpperCase()),
    },
    {
      label: "Education",
      value: nanny.education,
    },
  ];
  ...
  return (
    <article className="max-w-296 min-h-79.5 bg-white rounded-3xl p-6 relative">
      <div className="absolute top-6 right-6 flex flex-row gap-10">
        <div className="flex items-center gap-4 [&>span]:text-black-20">
          ...
          <p className={styles.infoStyle}>
            Price / 1 hour:{" "}
            <span className="text-bright-green">{nanny.price_per_hour}$</span>
          </p>
        </div>
      ...
      <div className="flex flex-row gap-6">
        ...
        <div className="flex flex-col gap-6 max-w-248">
          <div>
            ...
            <h3 className="font-medium text-2xl">{nanny.name}</h3>
          </div>
          ...
          <p className="text-grey text-[16px] ">{nanny.about}</p>

          {isExpanded && (
            <ul className="flex flex-col gap-6.25 mt-6">
              {nanny.reviews.map((item, index) => {
                return (
                  <li key={`r-${index}`} className="list-none">
                    <Review
      ...
      {isModalOpen && (
        <Appointment nanny={nanny} onClose={() => setIsModalOpen(false)} />
      )}
    </article>
  );
};
